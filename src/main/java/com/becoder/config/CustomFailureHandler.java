...
public class CustomFailureHandler extends SimpleUrlAuthenticationFailureHandler{
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
			...
			if(user.isEnable())
			{
				   if(user.isAccountNonLocked())
				   {
					   if(user.getFailedAttempt()< UserServiceImpl.ATTEMPT_TIME-1)
					   {
						...
						exception=new LockedException("your account is locked!! faild Attempt 3 time");
					}
				   }
					   ...
					   if(userService.unlockAccountTimeExpired(user))
					   {
						   exception=new LockedException("Account is unlocked! Please try to login");
						    ...
						    exception=new LockedException("Account is locked! Please try after");
					}
		...
		super.setDefaultFailureUrl("/sigin?error");
